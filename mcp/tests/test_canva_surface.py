...
def test_every_design_creation_tool_requires_a_template_id():
    manifest = _manifest()
    creation_tools = [t for t in manifest["tools"] if "create" in t["name"]]
...
def test_bulk_create_from_csv_tool_exists():
    manifest = _manifest()
    bulk_tools = [
        t
