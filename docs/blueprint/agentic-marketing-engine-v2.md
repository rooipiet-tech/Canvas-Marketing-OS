# Canvas Intelligence — Agentic Marketing Engine, blueprint v2
...
**127 specialist functions • 15-agent daily core • ratification model • earn-in autonomy**
improvement loops**

**Prepared for Pieter van Zyl** Canvas Intelligence · v1 22 July 2026 · **v2 4 September 2026** Johannesburg, South Africa
...
|-----------------------|-----------------------------------------------------------------------------------------|-------------------------------------------------------------------------|
| Agent count           | 127 defined functions (112 v1 + 15 ratification/earn-in functions, IDs 113–127 assigned in `docs/function-register.md` by PR 0); 15 core agents active daily | Completeness without running dozens of duplicated autonomous processes. |
...
IDs 113–127 are assigned from `docs/function-register.md` in the commit that lands this document (PR 0 of Appendix D — the next free block after the 112 v1 rows, so no provisional number changed). Each function's prompt, output schema and manifest exist in `functions/113-*` .. `functions/127-*`. Every function here emits an OptionCard (§C1) or a record consumed by one; none writes to `docs/permission-register.yaml`.
...
| Perplexity, Tavily, SerpAPI, Apify or Crawl4AI scraping stacks       | Claude’s native web research plus Semrush covers current monitoring needs. Third-party scraping adds terms-of-service risk and maintenance burden; revisit only if a specific, persistent monitoring gap emerges.                                                           |
...
| Design decisions — Agent count | 112 | 127 (IDs 113–127 assigned, PR 0) | New ratification/earn-in functions |
