# Canvas Intelligence — Agentic Marketing Engine, blueprint v4
...
**129 specialist functions • 15-agent daily core • ratification model • earn-in autonomy • continuous discovery • agent-bootstrapped sources**
improvement loops**

**Prepared for Pieter van Zyl** Canvas Intelligence · v1 22 July 2026 · v2 4 September 2026 · v3 4 September 2026 · **v4 4 September 2026** Johannesburg, South Africa
...
**v4 provenance.** v3 is never edited. v4 is v3 carried forward with one amendment: Stage 0 is no longer a hand-seed of the empty scan profiles. On 4 Sep 2026 an agent researched all twelve profiles from the open web (three probed options each, twelve named competitors, rejected sources with reasons) and produced twelve ratification cards; the ratifier chooses, PR 5a applies. Ruling (Pieter, 4 Sep): *no human seeds anything; the system researches the internet, completes the twelve scanners from scratch, then continuously searches and proposes.* Second ruling: the audit and road-to-completion are re-run from scratch against v4 (Appendix E), reversing v3's delta-only instruction. Change log under Appendix A, "v3 → v4".

**§0 baseline, v3 status (unchanged in v4).** v3 does **not** restate §0. Two of its numbers are unreconciled between two same-day reads of a table that should only grow (Brand Steward pass 59.9% → 64.1%; `unsupported-claim` 81 → 31; hypothesis: `caj-vault-retention-expiry`, unchecked). Until that is resolved the v2 §0 figures stand as *recorded on 4 Sep 09:49 UTC*, not as a settled baseline, and no v3 threshold is derived from them. Resolving the discrepancy is PR 5b's first task. One §0 fact is corrected here because it is a live fact, not a design choice: **9 of 12** scan profiles carry no source URLs, not 11 of 12 as first reported.

**Open decisions carried forward from v2, unchanged.** D1 founder video (record / synthetic / drop — recommendation: record); D2 approval budget (6 cards per working day shipped as the default; ratify or change before Appendix D PR 5); Canva refresh token (unlock C; blocks Fn 121); WordPress re-authorisation (unlock B; blocks s13-website); scan-profile source approval (ratify the twelve agent-researched bootstrap cards — see `claude_scan-profile-bootstrap-ratification-2026-09.md` — then `source.promote` cards under §11). v3 adds one: the choice of discovery API and crawler vendor — the ruling permits them; the vendor is an errand recorded in `docs/accepted-risks.md` at PR 5c.
...
| **ID** | **Agent / function** | **Mission** | **Core tasks** | **Primary KPIs** | **Self-improvement method** |
|---|---|---|---|---|---|
| 128 | **Source Discovery & Lifecycle Manager** (rewritten discovery-first; absorbs build-discovered `17-source-scout`) | Discover, probe, ratify, score and retire signal sources daily. | **Bootstrap mode (day one, no hand-seed) [v4]:** discovery for all twelve profiles at once → twelve `source.promote` cards with probe evidence → chosen sources land as `provisional`. Executed by agent 4 Sep 2026; see `source-candidates.bootstrap.yaml`. Daily per class: run the 11.2 query set; dedupe candidates against live sources and the retired list; probe (reachability, freshness, robots/ToS, allowlist status, signals yielded in window, duplicate rate, forecast yield); emit one `source.promote` card per class per day (3 candidates, declared axis, evidence = probe results, recommendation); chosen or SP-005-approved URL lands on the scan profile via the existing PR path. Nightly: yield row per live source (signals → cards → chosen; cost per chosen card; failures). Monthly: `source.retire` card per under-performing source with replacement on the same card; no default. Bootstrap sources are `provisional` and retire unless re-ratified within 30 days. | Live sources per profile (≥ 3); time-to-detection (target ≤ 24 h, blueprint starter); novelty score (≥ 25%); yield per source; `scan_profile_not_configured` = 0; Hit Rate on source cards. | Rejected candidates and retired sources retrain the probe scoring; class query-sets are versioned and A/B'd via Fn 127 evals. |
...
| Row / section | v3 | v4 | Why |
|---|---|---|---|
| Header, provenance | v3 beside v2 | v4 beside v3; one amendment | Versioning discipline: v3 was delivered, so this is a new version, not an edit |
| §11.3 Fn 128 | Gap-driven note: "does not replace the hand-seed" | Bootstrap mode: day-one discovery for all twelve profiles, twelve cards, no hand-seed; first bootstrap executed by agent 4 Sep and committed | Ruling: no human seeds anything |
| Appendix D | 5b, 5c | 5a added (apply ratified bootstrap; may precede PR 5), then 5b, 5c | Stage 0 becomes ratify-and-apply |
| Appendix E | Stage 0 "hand-seeded sources tagged provisional" | Stage 0 "apply agent-researched bootstrap"; unlock reworded | Same |
| Open decisions | "one-time ratification of the provisional seed" | "ratify the twelve bootstrap cards" | Same |
| Appendix E | Delta rows for audit and plan | Full re-derivation of both documents against v4 | Ruling 4 Sep, reversing "delta only": a bootstrap that changes the daily loop's behaviour invalidates carried-forward counts |
| New artefacts | — | `functions/_shared/source-candidates.bootstrap.yaml`; `claude_scan-profile-bootstrap-ratification-2026-09.md` | Evidence for the twelve cards |
...
| 5a [v4] | `bootstrap: apply ratified scan-profile sources` | Bind the twelve `profile_key`s in `functions/_shared/source-candidates.bootstrap.yaml` to the repo's real scan-profile ids; write the ratified option per profile as `provisional` (review_by +30 days). No human edits candidates; the file was agent-researched and ratified in chat 4 Sep. May land before PR 5. | `scan_profile_not_configured` = 0 over 3 days; `brief_published` 5/5 weekdays; every live source carries `provisional`, `ratified_by`, `ratified_at`, `review_by` |
| 5b | `Fn 128 + source-lifecycle-loop` | After 5a and after PR 5. Daily discovery per class via Claude web + Semrush only until 5c; `source.promote` / `source.retire` cards; nightly yield rows. Absorb `17-source-scout` (map or retire its ID). | First `source.promote` card in the digest; a chosen source lands on a profile via PR; yield rows nightly; provisional sources re-ratified or retired within 30 days |
...
# Appendix E — Re-derive the audit and the road to completion from scratch [v4]

Ruling (Pieter, 4 Sep, superseding the earlier "delta only"): the design-vs-built audit and the road-to-completion plan are **re-run end to end against v4**, not amended by delta rows. Both documents keep the format of their predecessors (`docs/audit-templates/` and the v2 editions) — same CSS, six-state scorecard (Built / Scaffolded / Partial / Blocked / Deferred / Drifted), evidence-cited rows, "what I could not verify" appendix — but every row is derived fresh against this document and a fresh baseline commit.

**Audit v4 must:**
- Take v4 as design of record; record the `main` commit as baseline; state the audit window.
- Rebuild the denominator from v4 with a counting rule per class and a reconciliation from 219: function register 129 (fn 40 superseded, `17-source-scout` absorbed into 128); loops 11; contracts C1–C5 (allowlist rule as C5); earn-in rules; standing-permission seeds SP-001–006; options_inbox components; discovery policies (`allowlist-rule`, `allowlist-deny`, `discovery-budget`); tech-stack integrations 15 (one discovery API, one crawler); approval budget; ratification metrics; the twelve bootstrap scan-profile bindings as their own class (each Built only when the profile carries a ratified, provisional-tagged source and the daily loop reaches it).
- Re-derive **all nine v1 classes independently** — the v2 audit carried five forward without re-census and said so; v4 does not carry anything forward.
- Re-run live verification fresh (read-only `az`, `caj-vault-query`): gate pass rates and violation histogram; the two unreconciled discrepancies (check `caj-vault-retention-expiry` execution history — resolve or state why not); `scan_profile_not_configured` and `brief_published` since PR 5a; discovery-trigger activity; `PUBLISHER_DRY_RUN`; Canva token; `smoke-test-v1` tagging (find the real field).
- Re-classify every deviation (DRIFT-1–15 plus `17-source-scout`) against v4.
- Human-input register scorecard for all 32 rows (H1–H32): does the human act still exist today (cite), which function/PR removes it, wiring state.
- Session process audit through whatever sessions exist.

**Road to N (v4) must:**
- Be written from scratch in the predecessor's structure (where we are; done bar; merged sequence; unlocks; schedule; risks), sized against what the repo has actually delivered since the last plan — including whether PR 5a landed and what it did to the daily loop.
- Keep Stage 0 (now: apply the ratified bootstrap) and the registry-dispatch pivot ahead of everything; then Appendix D PRs 1–5 (card mechanism), 5a/5b/5c, W1 + PRs 6/7 (instrument), 8/9 (corpus/voice), then the v1 fan-out interleaved with PRs 10–13.
- Extend the done bar with: emits a valid OptionCard; evidence refs resolve; no `default_on_timeout` on a downstream card; a source is live only if provisional-tagged and ratified; scraped content is stripped of instruction-shaped text before any prompt sees it.
- Carry the open decisions by name (D1, D2, Canva, WordPress, discovery vendor) and the one milestone: one real post through the governed path, approved by a chosen OptionCard with resolved evidence and no downstream default.
