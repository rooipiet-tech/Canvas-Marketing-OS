"""Kill switches (AC-12, AC-13, AC-24) — shared by Gatekeeper and Publisher.
...
for three reasons:
  ...
  2. The <5s propagation bound is trivially satisfied by a direct,
     uncached SELECT on the same connection the gate decision or publish
     attempt is already using. A Key Vault secret read would add a
     network round trip, throttling exposure and a cache-TTL temptation
     to the hot path of every single gate decision and publish attempt.
  3. Kill-switch state is not a secret. It is operational governance
     state that belongs next to gate_decisions/publish_attempts so an
     auditor can join "what was blocked" to "why" in one query.
...
very next decision or publish attempt, including one carrying a
pre-issued, still-valid gate token.

TD-08 EXTRACTION
-----------------
Until this package existed, this exact module was hand-duplicated as
services/gatekeeper/app/kill_switch.py and
services/publisher/app/kill_switch.py — kept honest only by
services/gatekeeper/tests/test_kill_switch_parity.py, which loaded both
files by path and asserted identical behaviour across the full
scope/function_id matrix. Both service copies now import this module
instead of re-declaring it; the parity test still passes, but is now
trivially true rather than a live drift detector. Gatekeeper and Publisher
are both BUNDLE-deployed (no Dockerfile, no sibling-package install
mechanism — see this package's own pyproject.toml header), so this source
file is embedded into both bundles via BUNDLE_MANIFEST.txt +
infra/main.bicep's loadTextContent, not pip-installed at runtime.
"""
