"""Client-reference clearance check for Client Advocacy Harvester (function 26).

Reads `docs/permission-register.yaml` and answers one question: may this
client name be used in public marketing output?
...
This module is a near-verbatim copy of `functions/02-brand-steward-qa/
permission_check.py`, kept code-for-code identical in logic (same
constants, same functions, same self-test) so that every function that
gates client naming shares one behaviour, not eight slightly different
re-implementations.

Run directly to execute the self-test:

    python functions/26-client-advocacy-harvester/permission_check.py
"""
...
def _self_test() -> None:
    ...
    assert listed.allowed is False, "Imperial is UNCLEARED and must block"
    assert listed.status == UNCLEARED, f"expected UNCLEARED for Imperial, got {listed.status}"
    ...
    print(f"Imperial            -> allowed={listed.allowed} status={listed.status}")
