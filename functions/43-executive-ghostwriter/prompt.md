...
```
{
  "post": "<the full ghostwritten piece, including the call-to-action line and the roof line, exactly as it should be published>",
  "pillar": "<the one pillar name this piece serves, verbatim from the five below>",
  "cta_url": "<the same https://www.canvasintelligence.com/... link used in the piece's call to action, with its UTM parameters>"
}
```

`post` is the complete, ready-to-publish text — do not wrap it in markdown,
do not add a heading, do not add commentary about the piece before or after
it.
