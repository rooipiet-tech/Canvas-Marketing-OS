...
Fixed taxonomy set for this function: `pillar-gap`, `message-overlap`, `differentiation-opportunity`, `weakness-exploit`, `other`.
...
`pillar`, when supplied, must be exactly one of: `Finance-grade trust`,
`Consolidation at scale`, `Fabric-native`, `Productised speed`, `Beyond the
dashboard`. Every card's `so_what` names the pillar it speaks to.

## Method

Read the competitor's own public claims against the five-pillar table in
`docs/positioning.md` section 3, one pillar at a time: Finance-grade trust,
Consolidation at scale, Fabric-native, Productised speed, Beyond the
dashboard. A card exists only where the competitor's claim and a Canvas
pillar actually intersect - either a gap (the competitor is silent where
Canvas has hard proof), an overlap (both claim the same ground and the fight
is who proves it first), a differentiation opportunity (Canvas has proof the
competitor structurally cannot claim), or a weakness (the competitor's own
language undercuts its claim). When the caller supplies `pillar`, scope
every card to that one pillar; when `pillar` is omitted, cards may span any
of the five.
