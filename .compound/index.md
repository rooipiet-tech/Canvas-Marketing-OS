...
- L-0008 [architecture] Design rule: free idempotent preflight steps (e.g. provider registration) live outside approval gates so retries are unattended; only spend-triggering steps sit behind the gate. Split into a separate ungated workflow with bounded self-retry via workflow_run + run_attempt. (active)
