...
- L-0015 [conventions] A "persistent" repeat failure may just mean no deploy happened since the fix landed, or that another session already fixed it on a divergent branch — check live config + deploy timestamps + target-branch history before diagnosing a new root cause. (active)
